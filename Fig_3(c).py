...
# =====================================================
# Random-degree critical R
# =====================================================
def get_randomdegree_R(R, n_nodes):

    nodes = list(R)

    random.shuffle(nodes)

    return set(nodes[:min(n_nodes, len(nodes))])
...
def run_experiment(network_type="BA"):
    ...
    for seed in range(n_runs):
        ...
        critical_R = get_randomdegree_R(R, shield_high)
        setups.append((T, K, R, critical_R))

        # Low target nodes
        T, K, R = manual_initial_TKR(G, targets_low)
        critical_R = get_randomdegree_R(R, shield_low)
        setups.append((T, K, R, critical_R))

        # Random target nodes
        T, K, R = manual_initial_TKR(G, targets_rand)
        critical_R = get_randomdegree_R(R, shield_rand)
        setups.append((T, K, R, critical_R))
