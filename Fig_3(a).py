...
# =====================================================
# Highest-degree critical R
# =====================================================
def get_high_degree_R(R, deg_dict, n_nodes):

    ranked = sorted(R, key=lambda x: deg_dict[x], reverse=True)
...
def run_experiment(network_type="BA"):
    ...
    for seed in range(n_runs):
        ...
        critical_R = get_high_degree_R(R, deg_dict, shield_high)
        setups.append((T, K, R, critical_R))

        # Low target nodes
        T, K, R = manual_initial_TKR(G, targets_low)
        critical_R = get_high_degree_R(R, deg_dict, shield_low)
        setups.append((T, K, R, critical_R))

        # Random target nodes
        T, K, R = manual_initial_TKR(G, targets_rand)
        critical_R = get_high_degree_R(R, deg_dict, shield_rand)
        setups.append((T, K, R, critical_R))
